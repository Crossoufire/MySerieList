...
from MyLists.models import Movies, MoviesActors, Series, SeriesList, Anime, AnimeActors, AnimeList, ListType, \
    SeriesActors, MoviesList, Status, RoleType, MoviesGenre, Games, GamesList, SeriesGenre, AnimeGenre, GamesGenre, \
    GamesCompanies, get_media_count
...
class MediaLists(object):
    ...
    def _get_media_count(self):
        ...
        self.count_total = self.media_list.query.filter_by(user_id=self.user.id).count()
        ...
        self.common_ids = [r[0].media_id for r in count_versus]

    def _base_query(self, A, B, C, D, E):
        if self.filter:
            self.query = db.session.query(A, B).join(A, A.id == B.media_id)\
                .filter(C == D, B.user_id == self.user.id, self.add_filter)\
                .order_by(self.add_sort).paginate(self.page, E, error_out=True)
        else:
            self.query = db.session.query(A, B).join(A, A.id == B.media_id) \
                .filter(C == D, B.user_id == self.user.id) \
                .order_by(self.add_sort).paginate(self.page, E, error_out=True)
    ...
    def _get_filter_query(self):
        ...
        self.add_filter = self.media_list.media_id.notin_(com_ids)

    def _base_search_query(self, A, B, C, D, E):
        self.query = db.session.query(A).join(B).join(C).filter(D) \
            .order_by(self.add_sort).paginate(self.page, E, error_out=True)

    def _get_sorting_query(self):
        if self.sort == 'title':
            self.add_sort = self.media.name
        elif self.sort == 'score':
            self.add_sort = self.media_list.score.desc()
        elif self.sort == 'comment':
            self.add_sort = self.media_list.comment.desc()
        elif self.sort == 'rewatch':
            self.add_sort = self.media_list.rewatched.desc()
        elif self.sort == 'release_date':
            self.add_sort = self.media_list.first_release_date.desc()

    def _get_category(self):
        try:
            self.category = Status(self.category)
            self.cat_value = self.category.value
        except:
            self.cat_value = self.category

    def _get_favorite_query(self):
        self._base_query((self.media, self.media_list), (self.media, self.media.id == self.media_list.media_id),
                         (self.media_list.favorite, self.media_list.user_id == self.user.id), 25)
        if self.filter:
            self._base_query((self.media, self.media_list), (self.media, self.media.id == self.media_list.media_id),
                             (self.media_list.favorite, self.media_list.user_id == self.user.id, self.add_filter), 25)

    def _get_not_favorite_not_search_query(self):
        self._base_query(self.media, self.media_list, self.media_list.status, self.category, 50)

    def _get_search_query(self):
        self.cat_value = "Search results for '{}'".format(self.search)
        if self.option == 'title':
            self._base_query((self.media, self.media_list), (self.media, self.media.id == self.media_list.media_id),
                             (or_(self.media.name.like('%' + self.search + '%'),
                                  self.media.original_name.like('%' + self.search + '%')),
                              self.media_list.user_id == self.user.id), 25)
            if self.filter == 'not_check':
                self._base_query((self.media, self.media_list), (self.media, self.media.id == self.media_list.media_id),
                                 (or_(self.media.name.like('%' + self.search + '%'),
                                      self.media.original_name.like('%' + self.search + '%')),
                                  self.media_list.user_id == self.user.id, self.add_filter), 25)
        elif self.option == 'name':
            self._base_query((self.media, self.media_list), (self.media, self.media.id == self.media_list.media_id),
                             (self.media.name.like('%' + self.search + '%'),
                              self.media_list.user_id == self.user.id), 25)
            if self.filter == 'not_check':
                self._base_query((self.media, self.media_list), (self.media, self.media.id == self.media_list.media_id),
                                 (self.media.name.like('%' + self.search + '%'),
                                  self.media_list.user_id == self.user.id, self.add_filter), 25)
        elif self.option == 'actor':
            self._base_search_query((self.media, self.media_list, self.actors_list),
                                    (self.media, self.media.id == self.media_list.media_id),
                                    (self.actors_list, self.actors_list.media_id == self.media_list.media_id),
                                    (self.actors_list.name.like('%' + self.search + '%'),
                                     self.media_list.user_id == self.user.id), 25)
            if self.filter == 'not_check':
                self._base_search_query((self.media, self.media_list, self.actors_list),
                                        (self.media, self.media.id == self.media_list.media_id),
                                        ...
                                        (self.actors_list.name.like('%' + self.search + '%'),
                                         self.media_list.user_id == self.user.id, self.add_filter), 25)
        elif self.option == 'genre':
            self._base_search_query((self.media, self.media_list, self.genre_list),
                                    (self.media, self.media.id == self.media_list.media_id),
                                    (self.genre_list, self.genre_list.media_id == self.media_list.media_id),
                                    (self.genre_list.genre.like('%' + self.search + '%'),
                                     self.media_list.user_id == self.user.id), 25)
            if self.filter == 'not_check':
                self._base_search_query((self.media, self.media_list, self.genre_list),
                                        (self.media, self.media.id == self.media_list.media_id),
    ...
    def recover_query(self, filter_val=False, sort_val='title', category=None, page=1, search=None, option='title'):
        self.page = page
        self.option = option
        self.sort = sort_val
        ...
        if not search and self.category != 'Favorite':
            self._get_not_favorite_not_search_query()
        elif self.category == 'Favorite':
            self._get_favorite_query()
        elif self.category == 'Search':
...
class AnimeLists(MediaLists):
    def __init__(self, user):
        ...
        self.actors_list = AnimeActors
        self.genre_list = AnimeGenre

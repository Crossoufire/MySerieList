...
def home():
    ...
    if login_form.validate_on_submit():
        ...
        if user and not user.active:
            current_app.logger.info('[{}] Connexion attempt while account not activated'.format(user.id))
            ...
            current_app.logger.info('[{}] Logged in'.format(user.id))
    ...
    if register_form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        current_app.logger.info('[{}] New account registration : username = {}, email = {}'
        ...
        if send_register_email(user):
            ...
            current_app.logger.error('[SYSTEM] Error while sending the registration email to {}'.format(user.email))
            abort(500)
...
@login_required
def logout():
    current_app.logger.info('[{}] Logged out'.format(current_user.id))
    logout_user()
...
def reset_password():
    ...
    if form.validate_on_submit():
        ...
        if send_reset_email(user):
            current_app.logger.info('[{}] Reset password email sent'.format(user.id))
            flash('An email has been sent with instructions to reset your password.', 'info')
            return redirect(url_for('auth.home'))
        else:
            current_app.logger.error('[SYSTEM] Error while sending the reset password email to {}'.format(user.email))
...
def reset_passord_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        current_app.logger.info('[{}] Password reset via reset password email'.format(user.id))
...
def register_account_token(token):
    ...
    db.session.commit()
    current_app.logger.info('[{}] Account activated'.format(user.id))
